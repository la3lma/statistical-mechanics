...
mechanics (\idx{Liouville's theorem}).  There is a quantum mechanical
...
\idx{Bolzmann's constant} traces back to the definition of temperature.
...
Joules.  That's just a historical fact.  \idx{Bolzmann} did not know the
...
the next year Susskind believe \idx{Einstein} figured out what Bolzmann's
...
didn't know his own constnt).  \idx{Newton} btw didn't know his own constant
...
mass, then it took some time before \idx{Milliken} figured out how to
...
\screenshot{deoverdslog2}{Energy change per bit of entropy}
...
called ``temperature''.   To state it colorfuly, it is the energy
...
Bolzmann distribution is the probability disrtibrution for the states
in the system \(A\).
