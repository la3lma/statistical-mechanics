...
own constnt).  \idx{Newton} btw. didn't know his own constant either.
...
called ``\idx{temperature}''.   To state it colorfully, it is {\idx the energy
...
These are the two important quantityes, let's calculate the
...
e^{-\beta E_i}\)\rmzXXX{Did I get that right? at 1:08:30 in lecture 3}, all that we are left with in the second term is
\(\log Z\).   Thus we have:
...
So the difference between the energy and the Helmholtz free energy is
